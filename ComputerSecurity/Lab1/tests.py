...
from encoders import VigenereEncoder
from hackers import Kasiski, Analyzer

with open('inputs/big.txt') as data_file:
    data = data_file.read()

report = open('output/report.md', 'w')
...
def run_test(text, keyword):
    encrypted = VigenereEncoder(keyword, LANG).encrypt(text)
